...
def Asses_Image_Quality(blur, noise, class1, class2, class3, cf):
    ...
    if blur < cf.getint("image_quality", "blur_threshold1") and noise < cf.getint("image_quality", "noise_threshold1"):
        Evaluation = class1
    elif blur > cf.getint("image_quality", "blur_threshold2") and noise > cf.getint("image_quality",
                                                                                    "noise_threshold2"):
        Evaluation = class2
    elif (cf.getint("image_quality", "blur_threshold1") - 10) < blur < (
            cf.getint("image_quality", "blur_threshold2") - 15) and (
            cf.getint("image_quality", "noise_threshold1") + 5) < noise < (
            cf.getint("image_quality", "noise_threshold2") + 5):
        Evaluation = class3
    else:
        Evaluation = '待质检'
...
if __name__ == '__main__':
    ...
    for image_path in imageNames:
        ...
        print("{} {} {}.  Evaluation:{}  ".format(image_path, str("%.4f" % blur_noise_Tenengrad),
                                                  str(blur_noise_Laplacian), Evaluation))
