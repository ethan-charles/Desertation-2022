...
class InverseCookingModel(nn.Module):
    ...
    def sample(self, img_inputs, greedy=True, temperature=1.0, beam=-1, true_ingrs=None, add_ingrs=None,
               override_sequence_length=0, top_k=10):
        ...
        # override or add to ingredients
        if true_ingrs is not None:
            ...
            outputs['ingr_ids'] = true_ingrs

        if add_ingrs is not None:
            combined_ingrs = torch.cat((add_ingrs, ingr_ids), 1)
            input_mask = mask_from_eos(combined_ingrs, eos_value=0, mult_before=False)
            combined_ingrs[input_mask == 0] = self.pad_value
            input_feats = self.ingredient_encoder(combined_ingrs)
            input_mask = input_mask.unsqueeze(1)
            outputs['ingr_ids'] = combined_ingrs

        ids, probs = self.recipe_decoder.sample(input_feats, input_mask, greedy, temperature, beam, img_features, 0,
                                                last_token_value=1, override_sequence_length=override_sequence_length,
                                                top_k=top_k)
